...
from chatterdocs.vectorstores import get_vectorstore, get_vectorstore_from_documents
...
LOADER_MAPPING = {
    ...
    "": (TextLoader, {"encoding": "utf8"})
    # Add more mappings for other file extensions and loaders as needed
}

# Load a single document from this of loader mappings
...
def load_documents(source_directory: str, ignored_files: List[str] = []) -> List[Document]:
    ...
    # load the documents sequentially
    pbar = tqdm(total=len(filtered_files), desc="Loading new documents", ncols=80)
    results = []
    for doc in filtered_files:
        results.extend(load_single_document(doc))
        pbar.update()
...
def process_documents(
    ...
    #text_splitter = TokenTextSplitter(chunk_size=125, chunk_overlap=12)
